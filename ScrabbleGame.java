...
public class ScrabbleGame
{
    ...
    private BoardChecker bc1 = new BoardChecker ();
    ...
    public void endTurn (){
        if (tile1.reDrawnTrue()&& tilesLeft>0){tile1= new Tile (0);}
        if (tile2.reDrawnTrue()&& tilesLeft>0){tile2= new Tile (1);}
        if (tile3.reDrawnTrue()&& tilesLeft>0){tile3= new Tile (2);}
        if (tile4.reDrawnTrue()&& tilesLeft>0){tile4= new Tile (3);}
        if (tile5.reDrawnTrue()&& tilesLeft>0){tile5= new Tile (4);}
        if (tile6.reDrawnTrue()&& tilesLeft>0){tile6= new Tile (5);}
        if (tile7.reDrawnTrue()&& tilesLeft>0){tile7= new Tile (6);}
        UI.println(tilesLeft);
        /**
         * Need to add a method which calculates the score of the board, it should only use double/tripple things if the tile over them is not 10
         * If the tile over them was 10 then basic letter score should be used even if the tile is over a double or triple
         * If tile is not 10 or 0 then the doubles and triples should be used
         * This way each round the non 0/10 are all moved to 10;
         * 
         */
